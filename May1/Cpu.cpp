...
std::vector<int> stats(20, 0);
// spot 0 will be context switches
// spots
...
void CPU::fetchAndExectuteInstructions()
{
    ...
    // }
    std::cout << std::endl;
    stats.erase(std::remove(stats.begin(), stats.end(), 0), stats.end());
    for (unsigned long i = 0; i < stats.size() - 1; i++)
    {
        std::cout << "Process: " << i << " had " << stats[i] << " clock cycles" << std::endl;
    }
    std::cout << "Number of context switches was: " << stats[stats.size() - 1] << std::endl;
}
...
void CPU::executeInstruction(int op, int o1, int o2, std::deque<PCB> &proQueue)
{
    ...
    case 0x11: // cmpi. dont change register values
        ...
        break;
    case 0x12: // cmpr
        // std::cout << "HERE" << registers[o1] << registers[o2] << std::endl;
    ...
    {
        // std::cout << "HERE" << std::endl;
        std::cout << "Process: " << proQueue.front().pageNumber << " complete, terminating process" << std::endl;
    ...
    case 0x22:                     // sleep
        stats[stats.size() - 1]++; // context switch
        stats[proTable.front().pageNumber] += proTable.front().clockCycles;
        // std::cout << "HERE1" << std::endl;
        // registers[o1] = 0;
        //  std::cout << registers[o1] << std::endl;
        if (registers[o1] == 2000000)
        {
            ...
            std::cout << "process sleeping!" << std::endl;
            proTable.front().state = "waiting";
            proTable.front().registerState = registers;
            // proTable.front().offset = offset;
            proTable.front().clockCycles = 0; // reset the clock
